...
构建产物 `paper/target/yeow-runtime-0.5.1.jar`（shaded，含 core + 引擎 + 配置解析）需要**复制到脚手架模板**，供 `create-yeow` 生成的插件项目使用：

```bash
cp paper/target/yeow-runtime-0.5.1.jar ../create-yeow/templates/default/.yeow/assets/
```
...
```bash
cp target/yeow-template-0.5.1.jar ../create-yeow/templates/default/.yeow/assets/
```
