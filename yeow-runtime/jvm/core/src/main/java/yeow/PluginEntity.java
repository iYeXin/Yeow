 ...
 * 由独立执行单元负责、存在行为与指标（如响应延迟）的实体。实现有两类：
 * JS 插件（{@link PluginThread}）与 Worker 虚拟插件（{@link WorkerThread}）；
 * JS 的特殊性（QuickJS 上下文、消息协议）只存在于实现内部。
 *
 ...
 * </ul>
 * 完成回报（如事件的 `event.complete`）由实现经 task 通道回传
 * （SyncCallbackHelper 契约）。实现可定义自己的消息格式，只要满足本接口语义。
 */
public interface PluginEntity {
    ...
    /**
     * 向插件投递消息。接受 JSON 字符串或 POJO（实现自行决定是否序列化：
