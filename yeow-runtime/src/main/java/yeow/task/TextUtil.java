...
public class TextUtil {
    ...
    /** 字面反斜杠的临时保护标记（NUL——正常文本中不会出现）：解析/序列化前后替换，
     *  使 `\n` `\b` 等字面反斜杠序列不被 MiniMessage/legacy 当作转义，也不被下游还原。 */
    private static final char BS_MARK = '\u0000';

    public static Component parse(String t) {
        if (t == null || t.isEmpty()) return Component.empty();
        var protectedText = t.replace("\\", String.valueOf(BS_MARK));
        try {
