...
**一个 service 的封装由两部分组成：**

| 部分 | 能力 |
| ---- | ---- |
| **服务端逻辑**（服务方） | `onRequest` 处理请求、`publish` 发布事件 |
| **客户端逻辑**（调用方） | `request` 请求服务、`subscribe` 订阅事件 |

**由于 public 服务同一时刻只能存在一个服务端**（重复注册被拒绝，其余插件自动降级为调用方），封装 npm 包时必须**隔离服务端与客户端**，典型代码结构：

1. **尝试注册服务端**——成功即成为唯一服务实例（持有 `token` 与 `onRequest`）；失败则降级，用 `err.serviceId` 接入既有服务
2. **以客户端身份封装对外接口**——对外只暴露 `serviceRequest` / `serviceSubscribe` 的封装函数，隐藏 serviceId/token 细节

**无论注册是否成功，暴露给客户端的逻辑应该一致**——使用者的插件不关心自己是否持有服务端，调用的都是同一套接口。

**绝对不允许对外暴露任何直接调用服务端能力的接口（最典型的就是发布事件）：**

- **注册不成功时拿不到 `token`**——依赖 `publish` 的对外接口直接逻辑缺失（调用无效果或报错）
- **即使设计不严谨让外部拿到了 `token`**——各插件运行在独立的 JS 上下文中，由调用方直接发布事件会造成**致命的状态不一致**：事件的业务含义（谁有权限发布、发布什么）只在服务端上下文中有意义，外部发布等于绕过服务端直接篡改业务状态

**最极端的例子**：哪怕需求只是**单纯发布一个事件**（如 `deposit` 成功后广播余额变化），也应在对外封装的实现中使用 `serviceRequest(svcId, '/publishEvent', event)`，由服务端配合处理该路径并 `publish`——发布始终发生在服务端内部。

具体规则：

1. **`token` 是服务方的私有凭证**，只在注册时返回一次，不应对外暴露。
2. **重复注册会拒绝 Promise**：`isPublic: true` 且同名服务已存在时，`registerService` / `registerNativeService` 抛出的 `Error` 携带 `serviceId` 字段（即已存在服务的 ID）。调用方应捕获错误，并用 `err.serviceId` 以**调用方身份**接入既有服务实例。
3. **npm 包封装模式**：公共服务通常来自一个 npm 依赖包（多个插件都会引入）。按包的角色分为三类（SDK / JS 服务 / 原生服务，可组合）——见下文 [Service 包类型](#service-包类型npm)。下面是一个"JS 服务"（类型 2）的示例，包同时是服务方与调用方：
...
> **二进制数据传输：** 请求/响应的 `body` 为 JSON，不支持直接传递二进制。需要传输二进制数据时，用 Base64 编码：`Uint8Array.prototype.toBase64()` / `Uint8Array.fromBase64()`（ES2026，引擎原生支持）。
