...
- **Yeow 生态权限检查（permissionCheck）**：`player.hasPermission` 任务与 Yeow 插件注册命令的**执行时检查**会先触发 `permissionCheck` 事件（[详见事件规范](/specifications/event/index#permissioncheckyeow-生态权限检查)）——handler 返回 `{ allowed }` 决定结果（**覆盖 Bukkit hasPermission**），不返回则回退 Bukkit；多 handler 返回冲突以最后返回的为准。**仅限 Yeow 生态**：其他 Java 插件的 hasPermission / 命令执行不经过此检查
