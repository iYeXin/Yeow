...
三大执行组件的**完整契约**（请求/响应格式、执行规则、同步异步语义、事件完成、命令补全）见各自专篇：

- [任务执行器与 task 通道](task/index.md) — 请求格式、优先级、同步 vs 异步、错误格式、224 个任务类型的逐条约定
- [事件监听](event/index.md) — 订阅/触发/完成、`eventId` 匹配、可取消事件、41 个事件的数据字段
- [命令执行器与补全](task/command.md) — 注册/执行/补全、sender 适配（Player / CONSOLE）、超时策略
