...
**触发时机：** 当游戏事件发生时，运行时向 `callbackId` 投递事件数据（`cb` 通道，`r` 为事件数据对象）。事件数据中包含 `_cancellable` 布尔字段标记是否可取消。
...
- **请求**：`{ "callbackId": "<cbId>", "mods": { "cancelled": <bool> } }`
- **返回**：`true`

| 字段             | 必填 | 说明                                                |
| ---------------- | ---- | --------------------------------------------------- |
| `callbackId`     | 是   | 与 `event.subscribe` 时传入的 `callbackId` 完全一致 |
...
3. 通过 cb 通道投递事件数据 → {t:"cb", p:callbackId, r:{...事件数据, _cancellable:bool}}
4. JS 端事件处理器执行
5. JS 端发送 event.complete → 运行时收到
6. 运行时应用 mods（如 cancellee）
```
