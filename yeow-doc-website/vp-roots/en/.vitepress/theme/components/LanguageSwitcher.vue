...
// Language switcher (matches VitePress native locale button style: globe icon).
// Dual-domain dual-build: yexin.wiki (EN) ↔ cn.yexin.wiki (CN), same base (/yeow/v1/),
// carries current path when switching languages.
// Uses VitePress reactive route (not window.location) so it works after client-side navigation.
const open = ref(false)
const rootEl = ref(null)
const route = useRoute()

const target = computed(() => {
  if (typeof window === 'undefined') return { label: '中文', href: '/' }
  const isCn = window.location.hostname === 'cn.yexin.wiki'
  // route.path already includes base prefix (/yeow/v1/...), no withBase() needed
...
<style>
/* Navbar right layout (matches native order): [Language Switcher][Dark Mode][GitHub] */
