...
**运行时配置目录保护**：实现**必须**拒绝所有 fs **写操作**（`writeFile` / `appendFile` / `writeBase64` / `appendBase64` / `delete` / `mkdir`）对运行时配置目录（如 `plugins/Yeow/runtime/`）的修改（全部级别一致拦截）——该目录存放运行时的 `config.yml`，插件不可通过 fs API 篡改。读取不受限。
