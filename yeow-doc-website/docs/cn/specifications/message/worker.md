...
// permissions 可选：{ "allow": [...], "deny": [...] }，默认继承主插件权限（deny 优先）
{ "t": "create", "p": { "name": "<worker名>", "code": "<代码>" | "entry": "<资源路径>", "msgCb": "<主插件侧回调id>", "permissions": { "allow": ["<模式>"], "deny": ["<模式>"] }, "cb": "<回调id>" } }
...
// 彻底销毁（卸载并移除注册；句柄作废，同名可重建）
{ "t": "destroy", "p": { "name": "<worker名>", "cb": "<回调id>" } }
...
| `destroy` | 彻底销毁：执行 `unload` 的全部清理并**从注册表移除**——**句柄作废，同名可重建** |
...
- Worker 与主插件**共享数据目录 / 资源**（fs 的 plugin 级 base、assets 命名空间一致）；权限默认继承主插件，可用 `permissions.allow` / `permissions.deny` 收紧（不可提权，deny 优先）
