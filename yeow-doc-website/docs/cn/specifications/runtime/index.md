...
插件与 **Worker（虚拟插件）** 都运行在**独立的单线程** JS 执行单元中（见[环境能力](../../environment.md)）。运行时**必须**满足以下两点：

1. **顺序与受控终止**：卸载、重载或关闭时，运行时必须保证**旧代码停止之后**新代码才开始执行；对无法自行停止的执行单元，必须以**受控**方式终止，且不得使宿主崩溃或长时间阻塞。
2. **优先优雅卸载**：应优先发送卸载消息，让执行单元自行退出——执行 `__yeowUnloadCbs` 中的全部回调后，经 `lifecycle` 通道发送 [`unloadDone`](../message/lifecycle.md#unloaddone)；而不是直接强制终止。

**示例流程（非规范，仅供参考）**

下面的四阶段流程是满足上述要求的一种典型实现，**平台可以不采用**；若采用，应满足其后的约束。

1. **优雅退出**：运行时向执行单元发出卸载请求（如 `DISABLE` / `RELOAD`），并在一个有界时间内等待 `unloadDone`。
2. **优雅退出失败**：等待超时仍未收到 `unloadDone`。
3. **强制终止（提供宽恕期）**：运行时请求**中断**。该请求在解释器检查点**以及** JS→Java 上行调用边界（`$send`）生效，并以**不可捕获**错误中止执行——JS 的 `catch` / `finally` **都不会执行**；同时唤醒可中断的宿主阻塞点。随后保留一个有界的**宽恕期**，让线程自行退出并回收其资源。
4. **遗弃并新建**：宽恕期结束仍未退出 → 该执行单元被**遗弃并隔离**：不再接收或投递任何消息 / 事件，其 `$send` 立即以不可捕获错误中止。若本次操作要求重载，运行时**创建全新的执行单元**继续运行。

采用示例流程时的约束：

- 中断只在检查点被观察；若线程卡在**不可中断的原生运算**（如灾难性回溯正则、超大 JSON 编解码），宽恕期内可能无法中止。
- 被遗弃的执行单元是**临时**的：卡住的调用一旦返回，它仍应正常回收（销毁其引擎上下文）；只有**永不返回**的运算才会永久泄漏。
- 宿主**绝不应**从其他线程销毁正在执行的引擎上下文（会导致 use-after-free / 进程崩溃）。
- 强制终止不可捕获，因此 JS 的 `catch` / `finally` **不保证执行**；清理逻辑应放在 `__yeowUnloadCbs`（正常卸载路径）。
...
注册一个回调函数，返回唯一回调 ID，格式为 `"cb_N"`（`N` 为自增整数，**每上下文随机基址**——热重载/重载创建新上下文后，回调 ID 不会与旧代重叠）。

> **跨代唯一性（为什么必须）**：运行时消息队列在重载时共享/存在竞态窗口——旧代消息（旧定时器投递、在途事件分发、迟到的异步结果）可能落入新上下文。若新旧代回调 ID 重叠（同一序号不同用途），旧消息会错误调用新回调（如事件 handler 收到其它事件/定时器载荷）。**回调 ID 必须全局唯一（至少跨代唯一）**，使旧代消息在新上下文查无此回调而被静默丢弃。
