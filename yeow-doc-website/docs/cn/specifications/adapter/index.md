...
| `{"t":"cb","p":"<cbId>","r":<data>,"eventId":"<eventId>"}` | 回调：事件数据 / 命令执行 / 命令补全 / 异步结果。`eventId` 为事件分发时生成的**每次唯一 id**，`r` 内同时携带 `_eventId`——JS 侧完成事件（`event.complete`）时应**原样回传** `eventId`，运行时用它精确匹配本次等待（同一 `cbId` 并发触发多次事件时防止 pend 覆盖） |
