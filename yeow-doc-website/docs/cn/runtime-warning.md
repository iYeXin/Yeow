...
> **虚拟插件（Worker）默认不触发本告警**：Worker 通常承载计算密集型任务，长时间占用 JS 线程不响应心跳属预期行为（`plugin.hung` 仍按原逻辑检测）。
...
发生挂起时，运行时按[卸载与强制终止](specifications/runtime/index.md#卸载与强制终止)处理（优先优雅卸载，失败则受控强制终止）：**优雅退出**（等待 `unloadDone`）→ 超时 → **强制终止**（不可捕获中断 + 宽恕期）→ **遗弃并隔离**（需要重载则新建执行单元）。

> 清理逻辑请放在 `onUnload`——强制终止不可捕获，JS 的 `catch` / `finally` 不保证执行。

**解决方案：** 排查 `onLoad` / `onInit` 中无出口的 `while(true)`；避免灾难性回溯正则与超大 JSON；繁重计算转移到原生服务；`/yeow reload` 或重启服务器。
...
通常不会——死循环不会自行退出。运行时按[卸载与强制终止](specifications/runtime/index.md#卸载与强制终止)处理：优雅退出 → 超时 → 不可捕获中断 + 宽恕期 → 遗弃并隔离（需要重载则新建执行单元）。纯 JS 死循环会被中断中止；卡在**不可中断的原生运算**（灾难性回溯正则、超大 JSON 等）则可能只能等待其返回（遗弃为临时，返回后自毁回收）。开发环境可热更新；生产环境 `/yeow reload` 或重启服务器。
